...
- Post-Setup Role Packs: `/post-setup/` — six English-language downloads for Nursing Student and Nursing Assistant, Staff Nurse and Quality Contributor, Nurse Leader and Manager, Nurse Educator and Instructional Designer, Nurse-Connected Ally, and the USA-only Nurse Practitioner. Each is used only after SOUL files and Hermes setup, and downloading does not install anything. Lanes 02 and 03 are governed self-install Hermes build kits: SHIFT declares 176 canonical checks with 20 optional SuperPowers inactive, and LEAD declares 113 canonical checks initially `Not Run` with 16 optional SuperPowers inactive and agents disabled. Hermes verifies the complete ZIP, performs read-only environment preflight, and presents one exact Implementation Activation Card before any local mutation. Lanes 01, 04, and 06 remain FUTURE, TEACH, and WINGS Complete Editions with exact combined activation-card gates. Lane 05 remains a review-first overlay. No lane becomes operational or institutionally authorized by publication, download, opening, or unzipping.
