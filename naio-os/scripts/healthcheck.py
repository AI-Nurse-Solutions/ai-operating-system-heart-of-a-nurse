...
passes. Checks the Phase 7 bundle is internally consistent.
"""
...
def main():
    parser = argparse.ArgumentParser(description="NAIO OS Phase 7 healthcheck")
    ...
    if not args.checksums_only:
        ...
        print("\n[5] Scripts")
        for s in ["scripts/preflight.sh", "scripts/import-soul.py", "scripts/import-projects.py", "scripts/render-profile.py", "scripts/self-test.py", "scripts/verify-release.py", "scripts/check-update.py", "scripts/healthcheck.py", "scripts/compute-checksums.sh", "install.sh", "bootstrap.sh"]:
            p = ROOT / s
            if p.is_file():
                ok(f"{s} present")
            else:
...
def print_summary():
    ...
    if FAIL > 0:
        ...
        print("\n✅ HEALTHCHECK PASSED — bundle is internally consistent (Phase 7 target-only apply scope).")
