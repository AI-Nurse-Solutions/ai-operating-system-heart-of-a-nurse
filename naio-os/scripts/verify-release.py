#!/usr/bin/env python3
"""
NAIO OS — verify-release.py (Phase 22)

Verifies the update-channel metadata and signed manifest before artifact
checksums are trusted. Phase 22 preserves signed release history, key-id trust metadata, rollback protection, partner briefing metadata, stewardship operating model metadata, and localization readiness metadata.
...
def main() -> int:
    parser = argparse.ArgumentParser(description="NAIO OS Phase 22 release verifier")
    ...
    failures = []
    say("\n=== NAIO OS — release verification (Phase 22) ===\n")
    ...
    if release.get("minimum_installer_phase") != 22:
        failures.append("release minimum_installer_phase must be 22")
