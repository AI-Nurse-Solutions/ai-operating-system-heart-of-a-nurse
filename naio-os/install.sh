#!/usr/bin/env bash
# =============================================================================
# NAIO OS — install.sh  (Phase 16: NIN community contribution flow)
# =============================================================================
...
print_help() {
  cat <<'EOF'
NAIO OS installer (Phase 16 — NIN contribution flow + healthcheck/self-test harness)
...
Options:
  ...
  --self-test        Run the Phase 16 built-in smoke test and exit.
...
cat <<'BANNER'
  ...
  ╔═══════════════════════════════════════════════════════════╗
  ║   NAIO OS — Nurse AI Operating System (Phase 16)           ║
...
if [[ $SELF_TEST -eq 1 ]]; then
  echo "▶ SELF-TEST — focused Phase 16 smoke test"
  exec python3 "$HERE/scripts/self-test.py"
fi

if [[ $CHECK_UPDATE -eq 1 ]]; then
  echo "▶ CHECK UPDATE — Phase 16 advisory, no mutation"
  exec python3 "$HERE/scripts/check-update.py"
fi

if [[ $RECOVERY_DRILL -eq 1 ]]; then
  echo "▶ RECOVERY DRILL — Phase 16 local-only snapshot/verify/extract/plan"
  if [[ -z "$TARGET" ]]; then
...
if [[ $ACTIVATION_CHECK -eq 1 ]]; then
  echo "▶ ACTIVATION CHECK — Phase 16 first-run readiness, no mutation"
  if [[ -z "$TARGET" ]]; then
...
if [[ $LAUNCH_CHECK -eq 1 ]]; then
  echo "▶ LAUNCH CHECK — Phase 16 public launch readiness, no mutation"
  if [[ -z "$TARGET" ]]; then
...
if [[ $COHORT_CHECK -eq 1 ]]; then
  echo "▶ COHORT CHECK — Phase 16 instructor/cohort readiness, no mutation"
  if [[ -z "$TARGET" ]]; then
...
if [[ $EVIDENCE_CHECK -eq 1 ]]; then
  echo "▶ EVIDENCE CHECK — Phase 16 EDENA evidence trail readiness, no mutation"
  if [[ -z "$TARGET" ]]; then
...
if [[ $CONTRIBUTION_CHECK -eq 1 ]]; then
  echo "▶ CONTRIBUTION CHECK — Phase 16 NIN community contribution readiness, no mutation"
  if [[ -z "$TARGET" ]]; then
...
if [[ $PILOT_CHECK -eq 1 ]]; then
  echo "▶ PILOT CHECK — Phase 16 institutional pilot readiness, no mutation"
  if [[ -z "$TARGET" ]]; then
...
if [[ $READINESS_CHECK -eq 1 ]]; then
  echo "▶ READINESS CHECK — Phase 16 EDENA readiness review, no mutation"
  if [[ -z "$TARGET" ]]; then
...
if [[ $REGISTRY_CHECK -eq 1 ]]; then
  echo "▶ REGISTRY CHECK — Phase 16 NAIO Agent Registry listing posture, no mutation"
  if [[ -z "$TARGET" ]]; then
...
echo "▶ STEP 6/8 — Plan"
cat <<'PLAN'
  Phase 16 maps EDENA into a Hermes-ready profile bundle and execution plane, with a one-line installer and built-in self-test:
...
if [[ $APPLY -eq 1 ]]; then
  ...
  ╔═══════════════════════════════════════════════════════════╗
  ║   ✅  NAIO OS Phase 16 apply complete.                     ║
  ║   Governed profile + execution templates rendered.        ║
  ╚═══════════════════════════════════════════════════════════╝

  Target: $TARGET
  Review README-FIRST.md, 10-Public-Launch/, 11-Cohort-Mode/, 12-Evidence-Trail/, and 13-Contribution-Flow/, and 14-Institutional-Pilot/, and 15-EDENA-Readiness/, and 16-Agent-Registry/ before copying, sharing, facilitating, documenting, contributing, piloting, requesting readiness review, or drafting registry listings.
  Doctrine: Agents propose. Humans judge. Nurses steward.
DONE
else
  cat <<'DONE'
  ...
  ╔═══════════════════════════════════════════════════════════╗
  ║   ✅  NAIO OS Phase 16 dry-run complete.                   ║
