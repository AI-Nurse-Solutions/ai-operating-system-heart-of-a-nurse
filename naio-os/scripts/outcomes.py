...
def check_profile(profile):
    ...
    warnings=[]
    if missing:
        failures.append('missing required outcomes files: '+', '.join(missing))
    runtime=load_yaml(profile/'config/edena-runtime.yaml') or {}
    rituals=load_yaml(profile/'cron/rituals.yaml') or {}
    outcomes=runtime.get('outcomes',{}) if isinstance(runtime,dict) else {}
    if runtime.get('version')!='2.0.0-phase23':
        failures.append(f"runtime version is not 2.0.0-phase23: {runtime.get('version')}")
    if outcomes.get('path')!='22-Adoption-Outcomes-Ledger/':
        failures.append('runtime outcomes.path is not 22-Adoption-Outcomes-Ledger/')
    if outcomes.get('outcomes_use')!='adoption_signal_and_learning_evidence_not_clinical_or_financial_outcomes_claims':
        failures.append('runtime outcomes_use must remain adoption signal/learning evidence only')
    for key in ['clinical_efficacy_claim','patient_outcome_improvement_claim','patient_safety_validation','quality_measure_impact_claim','roi_guarantee','staffing_reduction_claim','compliance_validation','competency_certification','performance_evaluation','research_determination','irb_determination','quality_improvement_approval','billing_or_reimbursement_claim','procurement_approval','deployment_approval','auto_score','auto_publish_dashboard','auto_send_report','auto_notify_manager','auto_evaluate_performance','auto_escalate','auto_submit_to_institution','auto_issue_badge','auto_issue_credential']:
        if outcomes.get(key) is not False:
            failures.append(f'runtime outcomes.{key} must be false')
    ...
    for phrase in required:
        if phrase.lower() not in combined.lower():
            failures.append(f'outcomes pack missing phrase: {phrase}')
    phi=hits(PHI_PATTERNS,combined)
    secrets=hits(SECRET_PATTERNS,combined)
    over=unsupported_overclaims(combined)
    if phi:
        failures.append('PHI-like content detected in outcomes pack: '+'; '.join(phi))
    if secrets:
        failures.append('secret-like content detected in outcomes pack: '+'; '.join(secrets))
    if over:
        failures.append('unsupported outcomes/impact overclaim detected: '+', '.join(sorted(set(over))))
...
def main():
    ap=argparse.ArgumentParser(description='Check Phase 22 Adoption & Outcomes Ledger Pack for a rendered NAIO profile bundle.')
    ap.add_argument('--profile',required=True)
    ap.add_argument('--json',action='store_true')
    args=ap.parse_args()
    profile=Path(args.profile).expanduser().resolve()
    if profile==Path.home() or str(profile) in ('/',str(Path.home()/'.hermes')):
        refuse('refusing to outcomes-check home or ~/.hermes directly')
