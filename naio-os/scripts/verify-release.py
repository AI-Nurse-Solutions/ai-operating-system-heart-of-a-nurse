#!/usr/bin/env python3
"""
NAIO OS — verify-release.py (Phase 9)

Verifies the update-channel metadata and signed manifest before artifact
checksums are trusted. Phase 9 preserves release history, key-id trust metadata, rollback protection, and adds activation metadata.
...
def main() -> int:
    parser = argparse.ArgumentParser(description="NAIO OS Phase 9 release verifier")
    ...
    failures = []
    say("\n=== NAIO OS — release verification (Phase 9) ===\n")
    ...
    if release.get("minimum_installer_phase") != 9:
        failures.append("release minimum_installer_phase must be 9")
