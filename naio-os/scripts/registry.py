...
def check_profile(profile):
    ...
    warnings=[]
    if missing:
        failures.append("missing required registry files: "+", ".join(missing))
    runtime=load_yaml(profile/'config/edena-runtime.yaml') or {}
    rituals=load_yaml(profile/'cron/rituals.yaml') or {}
    registry=runtime.get('registry',{}) if isinstance(runtime,dict) else {}
    if runtime.get('version')!='2.0.0-phase23':
        failures.append(f"runtime version is not 2.0.0-phase23: {runtime.get('version')}")
    if registry.get('path')!='16-Agent-Registry/':
        failures.append('runtime registry.path is not 16-Agent-Registry/')
    if registry.get('registry_use')!='human_reviewed_learning_registry_not_endorsement':
        failures.append('runtime registry_use must remain human-reviewed learning registry, not endorsement')
    for key in ['auto_vet','auto_list','auto_execute_agent','auto_recommend_for_patient_care']:
        if registry.get(key) is not False:
            failures.append(f'runtime registry.{key} must be false')
    ...
    for phrase in required:
        if phrase.lower() not in combined.lower():
            failures.append(f'registry pack missing phrase: {phrase}')
    phi=hits(PHI_PATTERNS,combined)
    secrets=hits(SECRET_PATTERNS,combined)
    over=unsupported_overclaims(combined)
    if phi:
        failures.append('PHI-like content detected in registry pack: '+'; '.join(phi))
    if secrets:
        failures.append('secret-like content detected in registry pack: '+'; '.join(secrets))
    if over:
        failures.append('unsupported registry endorsement/deployment overclaim detected: '+', '.join(sorted(set(over))))
...
def main():
    ap=argparse.ArgumentParser(description='Check Phase 16 NAIO Agent Registry pack for a rendered NAIO profile bundle.')
    ap.add_argument('--profile',required=True)
    ap.add_argument('--json',action='store_true')
    args=ap.parse_args()
    profile=Path(args.profile).expanduser().resolve()
    if profile==Path.home() or str(profile) in ('/',str(Path.home()/'.hermes')):
        refuse('refusing to registry-check home or ~/.hermes directly')
