...
- Post-Setup Role Packs: `/post-setup/` — five separate review-first downloads for Student Nurse, Staff Nurse, Nurse Leader and Manager, Nurse Educator, and Nurse-Connected Ally. Each is used only after SOUL files and Hermes setup; downloading does not install anything, and no memory, connector, cron, external action, or profile change is automatic.
- Latest Architecture Report: `/architecture-report.html` with stable PDF/Markdown downloads under `assets/nurse-ai-os-architecture-report.*` and date-stamped 2026-07-13 archive copies — documents the signed Governed Harness, `active_shadow` posture, five role lanes, four P0 architecture gaps, non-claims, and governed roadmap; it does not authorize canary enforcement, PHI, or clinical deployment.
