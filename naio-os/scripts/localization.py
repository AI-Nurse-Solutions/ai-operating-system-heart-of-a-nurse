...
def check_profile(profile):
    ...
    warnings=[]
    if missing:
        failures.append('missing required localization files: '+', '.join(missing))
    runtime=load_yaml(profile/'config/edena-runtime.yaml') or {}
    rituals=load_yaml(profile/'cron/rituals.yaml') or {}
    localization=runtime.get('localization',{}) if isinstance(runtime,dict) else {}
    if runtime.get('version')!='2.0.0-phase23':
        failures.append(f"runtime version is not 2.0.0-phase23: {runtime.get('version')}")
    if localization.get('path')!='21-Localization-Readiness/':
        failures.append('runtime localization.path is not 21-Localization-Readiness/')
    if localization.get('localization_use')!='adaptation_readiness_not_official_localization_or_jurisdictional_approval':
        failures.append('runtime localization_use must remain readiness/adaptation only, not official localization or jurisdictional approval')
    for key in ['legal_advice','regulatory_advice','compliance_determination','official_translation','translation_certification','localization_approval','cultural_validation','jurisdictional_approval','ministry_approval','institutional_endorsement','clinical_governance_authority','clinical_deployment_approval','procurement_approval','cross_border_data_transfer_approval','immigration_advice','licensure_advice','recruitment_activity','fundraising_solicitation','auto_translate','auto_publish_localized_materials','auto_contact_local_partners','auto_assign_local_stewards','auto_collect_regional_data','auto_submit_to_authorities']:
        if localization.get(key) is not False:
            failures.append(f'runtime localization.{key} must be false')
    ...
    for phrase in required:
        if phrase.lower() not in combined.lower():
            failures.append(f'localization pack missing phrase: {phrase}')
    phi=hits(PHI_PATTERNS,combined)
    secrets=hits(SECRET_PATTERNS,combined)
    over=unsupported_overclaims(combined)
    if phi:
        failures.append('PHI-like content detected in localization pack: '+'; '.join(phi))
    if secrets:
        failures.append('secret-like content detected in localization pack: '+'; '.join(secrets))
    if over:
        failures.append('unsupported localization authority/approval overclaim detected: '+', '.join(sorted(set(over))))
...
def main():
    ap=argparse.ArgumentParser(description='Check Phase 21 Localization / International Readiness Lane Pack for a rendered NAIO profile bundle.')
    ap.add_argument('--profile',required=True)
    ap.add_argument('--json',action='store_true')
    args=ap.parse_args()
    profile=Path(args.profile).expanduser().resolve()
    if profile==Path.home() or str(profile) in ('/',str(Path.home()/'.hermes')):
        refuse('refusing to localization-check home or ~/.hermes directly')
