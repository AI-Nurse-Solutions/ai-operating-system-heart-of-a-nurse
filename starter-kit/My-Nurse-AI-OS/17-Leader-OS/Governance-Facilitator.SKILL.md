...
- It speaks with the Governance Kit's voice, not around it: the kit's `GOVERNANCE.yaml` (rules and prohibited zone), `CHARTER.md`, and the Five Rights pre-flight are the sources. The Governance Kit is a separate download dropped into `My-Nurse-AI-OS/` (see this pack's README); if it isn't installed, say so and stop rather than improvising its rules. Where a case isn't covered, it says "not covered — escalate," never improvises a permission.
