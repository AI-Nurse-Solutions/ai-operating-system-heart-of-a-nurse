...
def check_profile(profile):
    ...
    warnings=[]
    if missing:
        failures.append('missing required partner files: '+', '.join(missing))
    runtime=load_yaml(profile/'config/edena-runtime.yaml') or {}
    rituals=load_yaml(profile/'cron/rituals.yaml') or {}
    partner=runtime.get('partner',{}) if isinstance(runtime,dict) else {}
    if runtime.get('version')!='2.0.0-phase23':
        failures.append(f"runtime version is not 2.0.0-phase23: {runtime.get('version')}")
    if partner.get('path')!='19-Partner-Briefing/':
        failures.append('runtime partner.path is not 19-Partner-Briefing/')
    if partner.get('partner_use')!='informational_partner_briefing_not_solicitation_or_approval':
        failures.append('runtime partner_use must remain informational, not solicitation or approval')
    for key in ['fundraising_solicitation','investment_advice','grant_application','contracting_authority','partnership_approval','sponsor_approval','institutional_endorsement','procurement_approval','clinical_deployment_approval','legal_advice','compliance_determination','auto_contact_partners','auto_send_materials','auto_follow_up','auto_create_data_room','auto_accept_funding']:
        if partner.get(key) is not False:
            failures.append(f'runtime partner.{key} must be false')
    ...
    for phrase in required:
        if phrase.lower() not in combined.lower():
            failures.append(f'partner pack missing phrase: {phrase}')
    phi=hits(PHI_PATTERNS,combined)
    secrets=hits(SECRET_PATTERNS,combined)
    over=unsupported_overclaims(combined)
    if phi:
        failures.append('PHI-like content detected in partner pack: '+'; '.join(phi))
    if secrets:
        failures.append('secret-like content detected in partner pack: '+'; '.join(secrets))
    if over:
        failures.append('unsupported partner/funding/approval overclaim detected: '+', '.join(sorted(set(over))))
...
def main():
    ap=argparse.ArgumentParser(description='Check Phase 19 Partner / Sponsor Briefing Pack for a rendered NAIO profile bundle.')
    ap.add_argument('--profile',required=True)
    ap.add_argument('--json',action='store_true')
    args=ap.parse_args()
    profile=Path(args.profile).expanduser().resolve()
    if profile==Path.home() or str(profile) in ('/',str(Path.home()/'.hermes')):
        refuse('refusing to partner-check home or ~/.hermes directly')
