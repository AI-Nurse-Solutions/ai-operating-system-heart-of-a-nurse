...
def check_profile(profile):
    ...
    for key in ['legal_advice','compliance_determination','clinical_governance_authority','institutional_approval','institutional_endorsement','procurement_approval','certification','credentialing','disciplinary_action','auto_approve_policy','auto_enforce_policy','auto_assign_members','auto_publish_minutes','auto_escalate']:
        if governance.get(key) is not False: failures.append(f'runtime governance.{key} must be false')
    ...
    for phrase in required:
        if phrase.lower() not in combined.lower(): failures.append(f'governance pack missing phrase: {phrase}')
    phi=hits(PHI_PATTERNS,combined); secrets=hits(SECRET_PATTERNS,combined); over=unsupported_overclaims(combined)
    if phi: failures.append('PHI-like content detected in governance pack: '+'; '.join(phi))
    if secrets: failures.append('secret-like content detected in governance pack: '+'; '.join(secrets))
    if over: failures.append('unsupported governance/authority overclaim detected: '+', '.join(sorted(set(over))))
...
def main():
    ap=argparse.ArgumentParser(description='Check Phase 18 Governance Board / Steward Council pack for a rendered NAIO profile bundle.'); ap.add_argument('--profile',required=True); ap.add_argument('--json',action='store_true'); args=ap.parse_args(); profile=Path(args.profile).expanduser().resolve()
    if profile==Path.home() or str(profile) in ('/',str(Path.home()/'.hermes')): refuse('refusing to governance-check home or ~/.hermes directly')
