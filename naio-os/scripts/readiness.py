#!/usr/bin/env python3
"""NAIO OS — readiness.py (Phase 15). Formative human review only; not certification."""
...
def check_profile(profile):
    ...
    warnings=[]
    if missing:
        failures.append("missing required readiness files: "+", ".join(missing))
    runtime=load_yaml(profile/'config/edena-runtime.yaml') or {}
    rituals=load_yaml(profile/'cron/rituals.yaml') or {}
    readiness=runtime.get('readiness',{}) if isinstance(runtime,dict) else {}
    if runtime.get('version')!='2.0.0-phase23':
        failures.append(f"runtime version is not 2.0.0-phase23: {runtime.get('version')}")
    if readiness.get('path')!='15-EDENA-Readiness/':
        failures.append('runtime readiness.path is not 15-EDENA-Readiness/')
    if readiness.get('readiness_use')!='formative_human_review_not_certification':
        failures.append('runtime readiness_use must remain formative human review, not certification')
    for key in ['auto_score','auto_pass_fail','auto_issue_badge','auto_issue_credential']:
        if readiness.get(key) is not False:
            failures.append(f'runtime readiness.{key} must be false')
    ...
    for phrase in required:
        if phrase.lower() not in combined.lower():
            failures.append(f'readiness pack missing phrase: {phrase}')
    phi=hits(PHI_PATTERNS,combined)
    secrets=hits(SECRET_PATTERNS,combined)
    over=unsupported_overclaims(combined)
    if phi:
        failures.append('PHI-like content detected in readiness pack: '+'; '.join(phi))
    if secrets:
        failures.append('secret-like content detected in readiness pack: '+'; '.join(secrets))
    if over:
        failures.append('unsupported certification/readiness overclaim detected: '+', '.join(sorted(set(over))))
...
def main():
    ap=argparse.ArgumentParser(description='Check Phase 15 EDENA readiness pack for a rendered NAIO profile bundle.')
    ap.add_argument('--profile',required=True)
    ap.add_argument('--json',action='store_true')
    args=ap.parse_args()
    profile=Path(args.profile).expanduser().resolve()
    if profile==Path.home() or str(profile) in ('/',str(Path.home()/'.hermes')):
        refuse('refusing to readiness-check home or ~/.hermes directly')
    ...
    if args.json:
        ...
        print('\n=== NAIO OS — Phase 15 EDENA readiness check ===\n')
        print(json.dumps(report,indent=2))
