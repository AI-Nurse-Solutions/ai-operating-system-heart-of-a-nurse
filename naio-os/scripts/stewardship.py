...
def check_profile(profile):
    ...
    warnings=[]
    if missing:
        failures.append('missing required stewardship files: '+', '.join(missing))
    runtime=load_yaml(profile/'config/edena-runtime.yaml') or {}
    rituals=load_yaml(profile/'cron/rituals.yaml') or {}
    stewardship=runtime.get('stewardship',{}) if isinstance(runtime,dict) else {}
    if runtime.get('version')!='2.0.0-phase23':
        failures.append(f"runtime version is not 2.0.0-phase23: {runtime.get('version')}")
    if stewardship.get('path')!='20-Stewardship-Operating-Model/':
        failures.append('runtime stewardship.path is not 20-Stewardship-Operating-Model/')
    if stewardship.get('stewardship_use')!='advisory_operating_model_not_institutional_authority':
        failures.append('runtime stewardship_use must remain advisory operating model, not institutional authority')
    for key in ['legal_advice','compliance_determination','procurement_approval','budget_approval','contracting_authority','institutional_policy_authority','clinical_governance_authority','clinical_deployment_approval','staffing_decision','labor_relations_decision','credentialing','certification','auto_implement','auto_assign_owners','auto_notify_stakeholders','auto_escalate','auto_publish_dashboard']:
        if stewardship.get(key) is not False:
            failures.append(f'runtime stewardship.{key} must be false')
    ...
    for phrase in required:
        if phrase.lower() not in combined.lower():
            failures.append(f'stewardship pack missing phrase: {phrase}')
    phi=hits(PHI_PATTERNS,combined)
    secrets=hits(SECRET_PATTERNS,combined)
    over=unsupported_overclaims(combined)
    if phi:
        failures.append('PHI-like content detected in stewardship pack: '+'; '.join(phi))
    if secrets:
        failures.append('secret-like content detected in stewardship pack: '+'; '.join(secrets))
    if over:
        failures.append('unsupported stewardship authority/implementation overclaim detected: '+', '.join(sorted(set(over))))
...
def main():
    ap=argparse.ArgumentParser(description='Check Phase 20 Institutional Stewardship Operating Model Pack for a rendered NAIO profile bundle.')
    ap.add_argument('--profile',required=True)
    ap.add_argument('--json',action='store_true')
    args=ap.parse_args()
    profile=Path(args.profile).expanduser().resolve()
    if profile==Path.home() or str(profile) in ('/',str(Path.home()/'.hermes')):
        refuse('refusing to stewardship-check home or ~/.hermes directly')
