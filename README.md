...
- BREATHE Respiratory Care Complete AI OS: `/respiratory-care/` — a standalone adjacent clinical lane for respiratory therapists and other respiratory care professionals. BREATHE is not a Nurse AI OS post-setup role or ROUNDS extension. It uses an isolated `respiratory_care` lane and `resp_breathe.*` namespace, 24 inactive optional SuperPowers, 10 `PERM-P0 Disabled` suggested agents, and 160 declared release and installation-acceptance checks. Downloading does not install anything; Private Respiratory OS accepts no PHI, real patient/event narrative, or device data, and has no device control or autonomous clinical action.
