...
def hits(patterns, text):
    ...
    for pat,name in patterns:
        m=pat.search(text)
        if m:
            found.append(f"{name}: {m.group(0)[:48]}")
    return found
...
def check_profile(profile: Path) -> dict:
    ...
    warnings=[]
    if missing:
        failures.append('missing required commercial activation files: '+', '.join(missing))
    runtime=load_yaml(profile/'config/edena-runtime.yaml') or {}
    rituals=load_yaml(profile/'cron/rituals.yaml') or {}
    commercial=runtime.get('commercial',{}) if isinstance(runtime,dict) else {}
    if runtime.get('version')!='2.0.0-phase23':
        failures.append(f"runtime version is not 2.0.0-phase23: {runtime.get('version')}")
    if commercial.get('path')!='23-Commercial-Activation/':
        failures.append('runtime commercial.path is not 23-Commercial-Activation/')
    if commercial.get('commercial_use')!='founding_steward_cohort_and_pilot_conversation_activation_not_procurement_or_clinical_deployment':
        failures.append('runtime commercial_use must remain founding cohort/pilot conversation activation only')
    for key in ['clinical_deployment_approval','procurement_approval','contracting_authority','legal_advice','compliance_determination','certification','credentialing','competency_validation','fundraising_solicitation','investment_advice','grant_application','auto_enroll','auto_accept_payment','auto_send_invoice','auto_contact_partners','auto_follow_up','auto_create_data_room','auto_accept_funding','auto_publish_dashboard','auto_report_to_institution','auto_notify_manager']:
        if commercial.get(key) is not False:
            failures.append(f'runtime commercial.{key} must be false')
    ...
    for phrase in required:
        if phrase.lower() not in combined.lower():
            failures.append(f'commercial pack missing phrase: {phrase}')
    phi=hits(PHI_PATTERNS,combined)
    secrets=hits(SECRET_PATTERNS,combined)
    over=unsupported_overclaims(combined)
    if phi:
        failures.append('PHI-like content detected in commercial pack: '+'; '.join(phi))
    if secrets:
        failures.append('secret-like content detected in commercial pack: '+'; '.join(secrets))
    if over:
        failures.append('unsupported commercial/authority overclaim detected: '+', '.join(sorted(set(over))))
    ...
    for r in runtime.get('cron_rituals',[]) if isinstance(runtime,dict) else []:
        if r.get('scheduled') is not False:
            scheduled.append(r.get('id'))
    if scheduled:
...
def main():
    ap=argparse.ArgumentParser(description='Check Phase 23 Commercial Activation Pack for a rendered NAIO profile bundle.')
    ap.add_argument('--profile',required=True)
    ap.add_argument('--json',action='store_true')
    args=ap.parse_args()
    profile=Path(args.profile).expanduser().resolve()
    if profile==Path.home() or str(profile) in ('/',str(Path.home()/'.hermes')):
        refuse('refusing to commercial-check home or ~/.hermes directly')
