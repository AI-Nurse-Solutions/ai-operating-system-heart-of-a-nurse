...
- a live post-setup distribution with six English-language role-specific downloads, including Nursing Student and Assistant, Nurse Leader, and USA-only Nurse Practitioner Complete Editions;
...
| Post-setup packages | Six live ZIPs: three review-first role overlays plus Nursing Student and Assistant, Nurse Leader, and USA-only NP Complete Editions; all checksum-verifiable and `not_installed` on download |
...
| Full SuperPowers module tree in review-first lanes 02, 04, and 05 | **No, not supplied in the three remaining original role ZIPs** | No | No | Separate future package decision | Reference-only gap; Nursing Student and Assistant, Nurse Leader, and NP lanes are separately complete and embedded |
...
Post-setup role resolver
           │
           ├─ Nursing Student and Nursing Assistant
           ├─ Staff Nurse
...
Download one role pack
                  │
                  ├─ Lanes 02, 04, 05: review-first preflight
                  │       └─ proposed changes, EDENA tiers, permissions,
                  │          conflicts, rollback → apply / revise / defer / reject
                  │
                  └─ Lanes 01, 03, 06: read-only Complete Edition preflight
...
Each live package includes a role manifest and declares `install_on_download: false`. Lanes 02, 04, and 05 use review-first approval. Lane 01 uses a read-only Nursing Student and Nursing Assistant FUTURE Complete Edition preflight, one exact combined activation card, a foundation-first/FUTURE-second sequence, 136 embedded release checks, three protected pathways, and eighteen inactive optional powers. Lane 03 uses the equivalent governed Nurse Leader sequence with 113 release checks. Lane 06 uses the equivalent governed NP sequence with 145 embedded acceptance tests and remains USA-only.
...
The three review-first lanes use a two-step review and explicit approval. The Nursing Student and Assistant, Nurse Leader, and NP Complete Editions require a read-only preflight and approval of an exact combined activation card before their governed multi-phase installation begins. Downloading or unzipping never starts either process.

## 8.3 Review-first SuperPowers reference limitation

The SuperPowers documents supplied for lanes 02, 04, and 05 referenced a larger distribution containing `manifest.yaml`, `core/`, `workflows/`, `templates/`, and `tests/`, but those modules were not supplied. Those three public packages therefore relabel the documents as `REFERENCE-SUPERPOWERS-*` and state that they must not be executed or used to invent missing modules.

This is the correct honesty posture for those three lanes. The Nursing Student and Assistant, Nurse Leader, and NP lanes are separately supplied Complete Editions with embedded runtime content and enumerated release checks; their inclusion does not constitute independent clinical or educational validation, accreditation, certification, compliance, or organizational authorization.
...
| **P1** | Missing full SuperPowers module tree in lanes 02, 04, and 05 | Those review-first role packs cannot truthfully install the complete referenced system | Keep references non-executable; build or formally remove full-pack promise |
...
- a live six-lane English post-setup distribution, including Nursing Student and Assistant, Nurse Leader, and USA-only Nurse Practitioner Complete Editions.
