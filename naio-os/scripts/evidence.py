...
def check_profile(profile: Path) -> dict:
    ...
    if runtime.get("version") != "2.0.0-phase15":
        failures.append(f"runtime version is not 2.0.0-phase15: {runtime.get('version')}")
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Check Phase 15 EDENA Evidence Trail readiness for a rendered NAIO profile bundle.")
    ...
    if args.json:
        ...
        print("\n=== NAIO OS — Phase 15 EDENA evidence trail check ===\n")
        print(json.dumps(report, indent=2))
