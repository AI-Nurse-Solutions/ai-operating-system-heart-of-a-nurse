...
def unsupported_overclaims(blob):
    ...
    for line in blob.splitlines():
        ...
        if not stripped or stripped.startswith('-') or stripped.startswith('>') or any(x in stripped for x in SAFE_NEGATIONS):
            continue
...
def check_profile(profile):
    ...
    warnings=[]
    if missing:
        failures.append("missing required orchestration files: "+", ".join(missing))
    runtime=load_yaml(profile/'config/edena-runtime.yaml') or {}
    rituals=load_yaml(profile/'cron/rituals.yaml') or {}
    orchestration=runtime.get('orchestration',{}) if isinstance(runtime,dict) else {}
    if runtime.get('version')!='2.0.0-phase23':
        failures.append(f"runtime version is not 2.0.0-phase23: {runtime.get('version')}")
    if orchestration.get('path')!='17-Florence-X-Orchestration/':
        failures.append('runtime orchestration.path is not 17-Florence-X-Orchestration/')
    if orchestration.get('orchestration_use')!='dry_run_preview_not_autonomous_multi_agent_runtime':
        failures.append('runtime orchestration_use must remain dry-run preview, not autonomous runtime')
    if orchestration.get('shared_memory_posture')!='orange_deferred_until_governed_review':
        failures.append('shared_memory_posture must remain orange_deferred_until_governed_review')
    for key in ['auto_handoff','auto_route','auto_execute_agents','auto_escalate','shared_memory_runtime']:
        if orchestration.get(key) is not False:
            failures.append(f'runtime orchestration.{key} must be false')
    ...
    for phrase in required:
        if phrase.lower() not in combined.lower():
            failures.append(f'orchestration pack missing phrase: {phrase}')
    phi=hits(PHI_PATTERNS,combined)
    secrets=hits(SECRET_PATTERNS,combined)
    over=unsupported_overclaims(combined)
    if phi:
        failures.append('PHI-like content detected in orchestration pack: '+'; '.join(phi))
    if secrets:
        failures.append('secret-like content detected in orchestration pack: '+'; '.join(secrets))
    if over:
        failures.append('unsupported orchestration/runtime overclaim detected: '+', '.join(sorted(set(over))))
...
def main():
    ap=argparse.ArgumentParser(description='Check Phase 17 Florence-X orchestration preview pack for a rendered NAIO profile bundle.')
    ap.add_argument('--profile',required=True)
    ap.add_argument('--json',action='store_true')
    args=ap.parse_args()
    profile=Path(args.profile).expanduser().resolve()
    if profile==Path.home() or str(profile) in ('/',str(Path.home()/'.hermes')):
        refuse('refusing to orchestration-check home or ~/.hermes directly')
