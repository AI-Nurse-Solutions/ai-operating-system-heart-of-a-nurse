#!/usr/bin/env bash
# =============================================================================
# NAIO OS — install.sh  (Phase 23: Commercial Activation)
# =============================================================================
...
print_help() {
  cat <<'EOF'
NAIO OS installer (Phase 23 — Commercial Activation + healthcheck/self-test harness)
...
Options:
  ...
  --self-test        Run the Phase 23 built-in smoke test and exit.
  ...
  --commercial-check     Verify Founding Steward Cohort / pilot commercial activation posture for --target.
  --help             Show this help.
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --commercial-check) COMMERCIAL_CHECK=1; shift ;;
...
cat <<'BANNER'
  ...
  ╔═══════════════════════════════════════════════════════════╗
  ║   NAIO OS — Nurse AI Operating System (Phase 23)           ║
...
if [[ $SELF_TEST -eq 1 ]]; then
  echo "▶ SELF-TEST — focused Phase 23 smoke test"
  exec python3 "$HERE/scripts/self-test.py"
fi

if [[ $CHECK_UPDATE -eq 1 ]]; then
  echo "▶ CHECK UPDATE — Phase 23 advisory, no mutation"
  exec python3 "$HERE/scripts/check-update.py"
fi

if [[ $RECOVERY_DRILL -eq 1 ]]; then
  echo "▶ RECOVERY DRILL — Phase 23 local-only snapshot/verify/extract/plan"
  if [[ -z "$TARGET" ]]; then
...
if [[ $ACTIVATION_CHECK -eq 1 ]]; then
  echo "▶ ACTIVATION CHECK — Phase 23 first-run readiness, no mutation"
  if [[ -z "$TARGET" ]]; then
...
if [[ $LAUNCH_CHECK -eq 1 ]]; then
  echo "▶ LAUNCH CHECK — Phase 23 public launch readiness, no mutation"
  if [[ -z "$TARGET" ]]; then
...
if [[ $COHORT_CHECK -eq 1 ]]; then
  echo "▶ COHORT CHECK — Phase 23 instructor/cohort readiness, no mutation"
  if [[ -z "$TARGET" ]]; then
...
if [[ $EVIDENCE_CHECK -eq 1 ]]; then
  echo "▶ EVIDENCE CHECK — Phase 23 EDENA evidence trail readiness, no mutation"
  if [[ -z "$TARGET" ]]; then
...
if [[ $CONTRIBUTION_CHECK -eq 1 ]]; then
  echo "▶ CONTRIBUTION CHECK — Phase 23 NIN community contribution readiness, no mutation"
  if [[ -z "$TARGET" ]]; then
...
if [[ $PILOT_CHECK -eq 1 ]]; then
  echo "▶ PILOT CHECK — Phase 23 institutional pilot readiness, no mutation"
  if [[ -z "$TARGET" ]]; then
...
if [[ $READINESS_CHECK -eq 1 ]]; then
  echo "▶ READINESS CHECK — Phase 23 EDENA readiness review, no mutation"
  if [[ -z "$TARGET" ]]; then
...
if [[ $REGISTRY_CHECK -eq 1 ]]; then
  echo "▶ REGISTRY CHECK — Phase 23 NAIO Agent Registry listing posture, no mutation"
  if [[ -z "$TARGET" ]]; then
...
if [[ $ORCHESTRATION_CHECK -eq 1 ]]; then
  echo "▶ ORCHESTRATION CHECK — Phase 23 Florence-X dry-run preview, no mutation"
  if [[ -z "$TARGET" ]]; then
...
if [[ $GOVERNANCE_CHECK -eq 1 ]]; then
  echo "▶ GOVERNANCE CHECK — Phase 23 advisory Steward Council posture, no mutation"
  if [[ -z "$TARGET" ]]; then
...
if [[ $PARTNER_CHECK -eq 1 ]]; then
  echo "▶ PARTNER CHECK — Phase 23 informational partner/sponsor briefing posture, no mutation"
  if [[ -z "$TARGET" ]]; then
...
if [[ $STEWARDSHIP_CHECK -eq 1 ]]; then
  echo "▶ STEWARDSHIP CHECK — Phase 23 advisory institutional operating model posture, no mutation"
  if [[ -z "$TARGET" ]]; then
...
if [[ $LOCALIZATION_CHECK -eq 1 ]]; then
  echo "▶ LOCALIZATION CHECK — Phase 23 localization / international readiness posture, no mutation"
  if [[ -z "$TARGET" ]]; then
...
if [[ $OUTCOMES_CHECK -eq 1 ]]; then
  echo "▶ OUTCOMES CHECK — Phase 23 adoption & outcomes ledger posture, no mutation"
  if [[ -z "$TARGET" ]]; then
...
if [[ $COMMERCIAL_CHECK -eq 1 ]]; then
  echo "▶ COMMERCIAL CHECK — Phase 23 founding cohort / pilot activation posture, no mutation"
  if [[ -z "$TARGET" ]]; then
    echo "❌ --commercial-check requires --target <rendered-profile-dir>" >&2
    exit 2
  fi
...
echo "▶ STEP 6/8 — Plan"
cat <<'PLAN'
  Phase 23 maps EDENA into a Hermes-ready profile bundle and execution plane, with a one-line installer and built-in self-test:
...
if [[ $APPLY -eq 1 ]]; then
  ...
  ╔═══════════════════════════════════════════════════════════╗
  ║   ✅  NAIO OS Phase 23 apply complete.                     ║
  ║   Governed profile + execution templates rendered.        ║
  ╚═══════════════════════════════════════════════════════════╝

  Target: $TARGET
  Review README-FIRST.md, 10-Public-Launch/, 11-Cohort-Mode/, 12-Evidence-Trail/, and 13-Contribution-Flow/, and 14-Institutional-Pilot/, and 15-EDENA-Readiness/, and 16-Agent-Registry/, and 17-Florence-X-Orchestration/, and 18-Governance-Board/, and 19-Partner-Briefing/, and 20-Stewardship-Operating-Model/, and 21-Localization-Readiness/ before copying, sharing, facilitating, documenting, contributing, piloting, requesting readiness review, drafting registry listings, rehearsing orchestration, convening governance review, briefing partners/sponsors, coordinating institutional stewardship, or preparing localized/international adaptations.
  Doctrine: Agents propose. Humans judge. Nurses steward.
DONE
else
  cat <<'DONE'
  ...
  ╔═══════════════════════════════════════════════════════════╗
  ║   ✅  NAIO OS Phase 23 dry-run complete.                   ║
