...
./naio-mc self-test   # 124 checks, including the ones that matter
...
All seven tabs are live. `self-test` runs 124 checks and attacks the rules rather than asserting them: it writes a malicious preset, probes for an endpoint that approves a gate, probes for one that schedules cron, kills a collector for real, and checks that promoting to memory left `SOUL.md` untouched byte-for-byte.
