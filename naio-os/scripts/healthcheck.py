...
passes. Checks the Phase 23 bundle is internally consistent.
"""
...
def main():
    parser = argparse.ArgumentParser(description="NAIO OS Phase 23 healthcheck")
...
def print_summary():
    ...
    if FAIL > 0:
        ...
        print("\n✅ HEALTHCHECK PASSED — bundle is internally consistent (Phase 23 target-only apply + activation + launch + cohort + evidence + contribution + pilot + readiness + registry + orchestration + governance + partner briefing + stewardship operating model + localization readiness + adoption outcomes ledger + commercial activation scope).")
