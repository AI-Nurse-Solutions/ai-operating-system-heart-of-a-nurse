...
def activation_report(profile: Path) -> tuple[dict, int]:
    ...
    if runtime.get("version") != "2.0.0-phase9":
        failures.append(f"runtime version is not 2.0.0-phase9: {runtime.get('version')}")
...
def main() -> int:
    ...
    if args.json:
        ...
        print("\n=== NAIO OS — Phase 9 activation check ===\n")
        print(json.dumps(report, indent=2))
        if code == 0:
